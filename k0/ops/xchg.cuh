...
static __device__ __noinline__ void op_xchg_push_utile(const Instr &in, const XchgPushArgs &a, char *) {
...
static __device__ MK_OPFN void op_xchg_push(const Instr &in, char *smem_u_) {
    XchgPushArgs a;
    __builtin_memcpy(&a, in.payload, sizeof(a));
    if (a.n_tokens != 1) { op_xchg_push_utile(in, a, smem_u_); return; }
...
static __device__ __noinline__ void op_xchg_reduce_utile(const Instr &in, const XchgReduceArgs &a, char *) {
...
static __device__ MK_OPFN void op_xchg_reduce(const Instr &in, char *smem_u_) {
    XchgReduceArgs a;
    __builtin_memcpy(&a, in.payload, sizeof(a));
    if (a.n_tokens != 1) { op_xchg_reduce_utile(in, a, smem_u_); return; }
    // decode fast path: the pre-U-loop body VERBATIM (codegen-identical)
